    ...
    if (ctx) {
        for (cs = ctx->cs; cs; cs = cs->next) {
            ...
            ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                "netcall: disconnect| detached ctx %p, cs %p", ctx, cs);
        }
    }
...
ngx_rtmp_netcall_create(ngx_rtmp_session_t *s, ngx_rtmp_netcall_init_t *ci)
{
    ...
    cs->timeout = ci->connect_timeout;
    cs->bufsize = ci->bufsize;
    cs->url = &u;
    cs->session = s;
    cs->filter = ci->filter;
    cs->sink = ci->sink;
    ...
    if (rc != NGX_OK && rc != NGX_AGAIN ) {
        ngx_log_debug0(NGX_LOG_DEBUG_RTMP, pc->log, 0,
...
ngx_rtmp_netcall_close(ngx_connection_t *cc)
{
    ...
    if (!cs->detached) {
        ...
        if (cs->handle && cs->handle(s, cs->arg, cs->in) != NGX_OK && s) {
