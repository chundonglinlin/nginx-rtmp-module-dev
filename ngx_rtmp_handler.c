...
ngx_rtmp_recv(ngx_event_t *rev)
{
    ...
    for( ;; ) {
        ...
        if (old_size) {
            ...
            s->ping_reset = 1;
            ngx_rtmp_update_bandwidth(&s->bw_in, n);
...
ngx_rtmp_send(ngx_event_t *wev)
{
    ...
    while (s->out_chain) {
        ...
        ngx_rtmp_update_bandwidth(&s->bw_out, n);
        ngx_rtmp_update_bandwidth(s->out[s->out_pos]->hdr.type
                                  == NGX_RTMP_MSG_VIDEO ?
                                  &s->bw_video : &s->bw_audio, n);
...
ngx_rtmp_handler_get_diff(size_t gop_pos, size_t gop_last,
        ...
        ngx_uint_t priority)
{
    ...
    nmsg = ngx_rtmp_handler_get_diff(s->out_pos, s->out_last, s->out_queue, 1);
