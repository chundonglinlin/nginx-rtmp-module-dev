...
ngx_int_t
ngx_live_create_hls_ctx(ngx_hls_session_t *hs)
{
    ngx_hls_cmd_ctx_t        *ctx, **pctx;

    ctx = ngx_rtmp_get_module_ctx(hs, ngx_hls_cmd_module);
    if (ctx == NULL) {
        ctx = ngx_pcalloc(hs->pool, sizeof(ngx_hls_cmd_ctx_t));
        if (ctx == NULL) {
            return NGX_ERROR;
        }

        ngx_rtmp_set_ctx(hs, ctx, ngx_hls_cmd_module);
    }

    ctx->session = hs;
    pctx = &hs->live_stream->hls_play_ctx;
...
void
ngx_live_delete_hls_ctx(ngx_hls_session_t *hs)
{
    ngx_hls_cmd_ctx_t        *ctx, **pctx;
...
ngx_int_t
ngx_live_create_mpegts_ctx(ngx_rtmp_session_t *s)
{
    ngx_rtmp_mpegts_ctx_t        *ctx, **pctx;

    ctx = ngx_rtmp_get_module_ctx(s, ngx_rtmp_mpegts_module);
    if (ctx == NULL) {
        ctx = ngx_pcalloc(s->connection->pool, sizeof(ngx_rtmp_mpegts_ctx_t));
        if (ctx == NULL) {
            ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "ngx-live: create_mpegts_ctx| alloc mpegts ctx failed");
            return NGX_ERROR;
        }
    ...
    ctx->session = s;
    pctx = &s->live_stream->hls_publish_ctx;

    /* only one publisher, for now */
    if (*pctx) {
        ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                      "ngx-live: create_mpegts_ctx| "
                      "publish ctx is exist, old ctx=%p", *pctx);
        return NGX_ERROR;
    }
...
void
ngx_live_delete_mpegts_ctx(ngx_rtmp_session_t *s)
{
    ngx_rtmp_mpegts_ctx_t        *ctx, **pctx;
...
static ngx_int_t
ngx_live_sort_priority_ctx(ngx_rtmp_session_t *s, ngx_live_stream_t *pst)
{
    ngx_rtmp_core_ctx_t          *ctx, *pctx;
    ngx_int_t                     priority;
    ...
    if (priority > pst->publish_ctx->session->priority) {
        ctx->next = pst->publish_ctx;
        pst->publish_ctx = ctx;
        return NGX_OK;
    }

    for (pctx = pst->publish_ctx; pctx->next; pctx = pctx->next) {
        if (priority < pctx->session->priority &&
            priority > pctx->next->session->priority)
        {
            ctx->next = pctx->next;
            pctx->next = ctx;
            return NGX_OK;
...
ngx_int_t
ngx_live_handle_priority_stream(ngx_rtmp_session_t *s, ngx_int_t publishing,
        ngx_int_t priority)
{
    ngx_live_stream_t          *pst;
    ngx_rtmp_core_ctx_t        *ctx;

    pst = ngx_live_fetch_stream(&s->serverid, &s->stream);

    if (priority != 0 && publishing && pst && pst->publish_ctx) {
        s->live_stream = pst;
        ...
        if (ngx_live_sort_priority_ctx(s, pst) == NGX_ERROR) {
            return NGX_ERROR;
        }
        return NGX_OK;
    }
