...
static char *ngx_rtmp_core_application(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_rtmp_core_custom_message(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
static const char *ngx_rtmp_custom_message_flag[] = {
    "none",
    "all",
    "part",
};
...
static ngx_command_t  ngx_rtmp_core_commands[] = {
    ...
    { ngx_string("variables_hash_max_size"),
      ...
      ngx_conf_set_num_slot,
      NGX_RTMP_MAIN_CONF_OFFSET,
      offsetof(ngx_rtmp_core_main_conf_t, variables_hash_max_size),
      NULL },
    ...
    { ngx_string("variables_hash_bucket_size"),
      ...
      ngx_conf_set_num_slot,
      NGX_RTMP_MAIN_CONF_OFFSET,
      offsetof(ngx_rtmp_core_main_conf_t, variables_hash_bucket_size),
      NULL },

    { ngx_string("bandwidth_dynamic_rtmp"),
      NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_flag_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_core_app_conf_t, bandwidth_dynamic),
      NULL },

    { ngx_string("tcp_cost_makeup"),
      NGX_RTMP_MAIN_CONF|NGX_RTMP_SRV_CONF|NGX_RTMP_APP_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_flag_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_core_app_conf_t, tcp_cost_makeup),
      NULL },
    ...
    { ngx_string("media_filter"),
      ...
      ngx_conf_set_flag_slot,
      NGX_RTMP_APP_CONF_OFFSET,
      offsetof(ngx_rtmp_core_app_conf_t, media_filter),
      NULL },

    { ngx_string("custom_message"),
      NGX_RTMP_MAIN_CONF|NGX_CONF_1MORE,
      ngx_rtmp_core_custom_message,
      NGX_RTMP_MAIN_CONF_OFFSET,
      0,
      NULL },
...
ngx_rtmp_core_create_main_conf(ngx_conf_t *cf)
{
    ...
    cmcf->custom_message_flag = NGX_CONF_UNSET_UINT;
...
ngx_rtmp_core_init_main_conf(ngx_conf_t *cf, void *conf)
{
    ...
    ngx_conf_init_uint_value(cmcf->custom_message_flag,
...
ngx_rtmp_core_create_app_conf(ngx_conf_t *cf)
{
    ...
    conf->bandwidth_dynamic = NGX_CONF_UNSET;
    conf->tcp_cost_makeup = NGX_CONF_UNSET;
    conf->media_filter = NGX_CONF_UNSET;
...
ngx_rtmp_core_merge_app_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->bandwidth_dynamic, prev->bandwidth_dynamic, 0);
    ngx_conf_merge_value(conf->tcp_cost_makeup, prev->tcp_cost_makeup, 0);
...
ngx_rtmp_core_application(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cacf->name.len == sizeof("*_org") - 1 &&
        ngx_strncasecmp(cacf->name.data, (u_char *)"*_org", cacf->name.len) == 0)
    {  /* org_app */
        cscf->org_app = cacf;
    }
...
static char *
ngx_rtmp_core_custom_message(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_rtmp_core_main_conf_t  *cmcf = conf;
    ngx_str_t                  *value, *message_name;
    ngx_uint_t                  n;
    ...
    for (n = 0; n < NGX_RTMP_CUSTOM_MESSAGE_MAX; n++) {
        if (ngx_strncmp(value[1].data, ngx_rtmp_custom_message_flag[n],
                    value[1].len) == 0)
        {
            cmcf->custom_message_flag = n;
        }
    }
    ...
    if (ngx_array_init(&cmcf->message_name, cf->pool,
            cf->args->nelts - 2, sizeof(ngx_str_t)) != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }

    message_name = ngx_array_push_n(&cmcf->message_name, cf->args->nelts - 2);
    if (message_name == NULL) {
        return NGX_CONF_ERROR;
    }

    for (n = 2; n < cf->args->nelts; n++) {
        *message_name++ = value[n];
    }
