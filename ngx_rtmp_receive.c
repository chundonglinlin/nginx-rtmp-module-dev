        ...
        ngx_rtmp_header_t *h, ngx_chain_t *in)
{
    ...
    if (ch && ch->nelts) {
        ...
        in->buf->pos -= act.offset;
        ngx_rtmp_live_amf_data_send(s, h, in, func);
        ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
...
ngx_int_t
ngx_rtmp_receive_meta(ngx_rtmp_session_t *s, ngx_array_t *meta, ngx_chain_t *in)
{
    ngx_rtmp_amf_ctx_t     act;
    ...
    return ngx_rtmp_amf_read_meta(&act, meta, s->connection->pool);
}
