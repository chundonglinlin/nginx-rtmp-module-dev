...
static u_char ngx_rtmp_cntv_mpegts_aac_header[] = {
...
static ngx_int_t
ngx_rtmp_cntv_mpegts_write_file(ngx_rtmp_cntv_mpegts_file_t *file, u_char *in,
    size_t in_size)
{
    u_char   *out;
    size_t    out_size, n;
    ...
    if (!file->encrypt) {
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, file->log, 0,
                       "cntv_mpegts: write %uz bytes", in_size);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, file->log, 0,
                   "cntv_mpegts: write %uz encrypted bytes", in_size);
...
static ngx_int_t
ngx_rtmp_cntv_mpegts_write_header(ngx_rtmp_cntv_mpegts_file_t *file)
{
    return ngx_rtmp_cntv_mpegts_write_file(file, ngx_rtmp_cntv_mpegts_aac_header,
                                      sizeof(ngx_rtmp_cntv_mpegts_aac_header));
}


static u_char *
ngx_rtmp_cntv_mpegts_write_pcr(u_char *p, uint64_t pcr)
{
    *p++ = (u_char) (pcr >> 25);
    *p++ = (u_char) (pcr >> 17);
    *p++ = (u_char) (pcr >> 9);
...
static u_char *
ngx_rtmp_cntv_mpegts_write_pts(u_char *p, ngx_uint_t fb, uint64_t pts)
{
    ngx_uint_t val;
...
ngx_int_t
ngx_rtmp_cntv_mpegts_write_frame(ngx_rtmp_cntv_mpegts_file_t *file,
    ngx_rtmp_cntv_mpegts_frame_t *f, ngx_buf_t *b)
{
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_CORE, file->log, 0,
                   "cntv_mpegts: pid=%ui, sid=%ui, pts=%uL, "
    ...
    while (b->pos < b->last) {
        ...
        *p++ = 0x10 | (f->cc & 0x0f); /* payload */
        ...
        if (first) {
            ...
            if (f->key) {
                ...
                *p++ = 0x50; /* random access + PCR */
                ...
                p = ngx_rtmp_cntv_mpegts_write_pcr(p, f->dts - NGX_RTMP_HLS_DELAY);
            }
            ...
            *p++ = (u_char) header_size;
            ...
            p = ngx_rtmp_cntv_mpegts_write_pts(p, flags >> 6, f->pts +
                                                         NGX_RTMP_HLS_DELAY);

            if (f->dts != f->pts) {
                p = ngx_rtmp_cntv_mpegts_write_pts(p, 1, f->dts +
                                                    NGX_RTMP_HLS_DELAY);
            }
        ...
        rc = ngx_rtmp_cntv_mpegts_write_file(file, packet, sizeof(packet));
        if (rc != NGX_OK) {
            return rc;
        }
    }
...
ngx_int_t
ngx_rtmp_cntv_mpegts_init_encryption(ngx_rtmp_cntv_mpegts_file_t *file,
...
ngx_int_t
ngx_rtmp_cntv_mpegts_open_file(ngx_rtmp_cntv_mpegts_file_t *file, u_char *path,
    ngx_log_t *log)
{
    file->log = log;
    ...
    if (ngx_rtmp_cntv_mpegts_write_header(file) != NGX_OK) {
        ngx_log_error(NGX_LOG_ERR, log, ngx_errno,
...
ngx_int_t
ngx_rtmp_cntv_mpegts_close_file(ngx_rtmp_cntv_mpegts_file_t *file)
{
    u_char   buf[16];
    ssize_t  rc;
