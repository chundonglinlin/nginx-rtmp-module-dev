...
* @param cdnIP       调度到CDN服务器的ip,自建CDN需要跟本机ip比较,外包CDN需要与给其预分配的编号比较
* @param streamID    流ID
* @param pEncryptData key值
* @param lDataLen     key长度
* @param flag         校验标志 相应的位为1表示检查,0表示不检查
从最低位起，第一位检查magicnum，第二位检查时间戳，第三位检查user ip，第四位检查filename, 第五位检查level 第六位检查CDN IP
...
-4:检查用户IP失败 -5:检查文件名失败 -6:检查level失败 -7:CDN ip检查失败 -8:cookietime过长(超过一天)
*/
