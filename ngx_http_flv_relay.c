...
static ngx_int_t
ngx_http_relay_send_request(ngx_rtmp_session_t *s, ngx_client_session_t *cs, unsigned static_pull)
{
    ngx_http_request_t         *hcr;
	...
	if(static_pull) {
		*ngx_cpymem(request_url.data, ctx->tc_url.data, ctx->tc_url.len) = 0;
		request_url.len = ctx->tc_url.len;
		
	} else if (ctx->pargs.len) {
...
#undef NGX_RTMP_DEFAULT_STR
    ...
    /* send http request */
    ngx_http_relay_send_request(rs, cs, s->static_pull_fake);
