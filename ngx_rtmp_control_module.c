...
{
    ngx_str_t                addr, *paddr, clientid, groupid;
    ngx_str_t                ip_port, ip_port_temp, remote_ip;
    ...
    if (ngx_http_arg(r, (u_char *) "ip_port", sizeof("ip_port") - 1,
            &ip_port_temp) == NGX_OK)
    {
        ip_port.data = ngx_pcalloc(r->connection->pool, ip_port_temp.len);
        ngx_memcpy(ip_port.data, ip_port_temp.data, ip_port_temp.len);
        ip_port.len = ip_port_temp.len;

        dst = ip_port.data;
        src = ip_port.data;
        ngx_unescape_uri(&dst, &src, ip_port.len, NGX_UNESCAPE_URI);

        addr_len = (ip_port.data + ip_port.len) - src;
        if (addr_len) {
            dst = ngx_movemem(dst, src, addr_len);
        }

        ip_port.len = dst - ip_port.data;
        ngx_rtmp_get_remoteaddr(s->connection, &remote_ip);
        if (remote_ip.len != ip_port.len ||
                ngx_strncmp(remote_ip.data, ip_port.data, ip_port.len))
        {
            ngx_pfree(r->connection->pool, ip_port.data);
            return NGX_CONF_OK;
        }
        ngx_pfree(r->connection->pool, ip_port.data);
    }

    if (ngx_http_arg(r, (u_char *) "groupid", sizeof("groupid") - 1, &groupid)
            == NGX_OK)
    {
        if (s->groupid.len != groupid.len ||
                ngx_strncmp(s->groupid.data, groupid.data, groupid.len))
        {
            return NGX_CONF_OK;
        }
    }
...
ngx_rtmp_control_walk_server(ngx_http_request_t *r, ngx_live_server_t *srv)
{
    ...
    stream.len = serverid.len + 1 + app.len + 1 + name.len; /* serverid/app/name */
...
ngx_rtmp_control_handler(ngx_http_request_t *r)
{
    ...
    if (ngx_http_arg(r, (u_char *) "secname", sizeof("secname") - 1, &section) != NGX_OK) {
        return NGX_ERROR;
    }

    if (ngx_http_arg(r, (u_char *) "method", sizeof("method") - 1, &method) != NGX_OK) {
        return NGX_ERROR;
    }
