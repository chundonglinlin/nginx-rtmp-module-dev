...
ngx_rtmp_mpegts_write_header(ngx_rtmp_mpegts_file_t *file)
{
    ...
    rc = -1;
    if (0 == file->audio_only && 0 == file->video_only) {
        /* video + audio */
        if (file->audio_type == TS_AUDIO_TYPE_AAC) {
            if (file->video_type == TS_VIDEO_TYPE_H264) {
                /* h264 + aac  .ts */
                rc = ngx_rtmp_mpegts_write_file(file, ngx_rtmp_mpegts_header,
                                sizeof(ngx_rtmp_mpegts_header));
            } else if (file->video_type == TS_VIDEO_TYPE_H265) {
                /* h265 + aac  .ts */
                rc = ngx_rtmp_mpegts_write_file(file,
                                ngx_rtmp_mpegts_hevc_aac_header,
                                sizeof(ngx_rtmp_mpegts_hevc_aac_header));
            }
        } else if (file->audio_type == TS_AUDIO_TYPE_MP3) {
            if (file->video_type == TS_VIDEO_TYPE_H264) {
                /* h264 + mp3  .ts */
                rc = ngx_rtmp_mpegts_write_file(file,
    ...
    } else if (1 == file->audio_only) {
        /* pure audio */
        if (file->audio_type == TS_AUDIO_TYPE_AAC) {
            /* aac  .ts */
            rc = ngx_rtmp_mpegts_write_file(file, ngx_rtmp_mpegts_aac_header,
                                        sizeof(ngx_rtmp_mpegts_aac_header));
        } else if (file->audio_type == TS_AUDIO_TYPE_MP3) {
             /* mp3  .ts */
            rc = ngx_rtmp_mpegts_write_file(file, ngx_rtmp_mpegts_mp3_header,
                                        sizeof(ngx_rtmp_mpegts_mp3_header));
        }
    } else if (1 == file->video_only) {
        /* pure video */
        if (file->video_type == TS_VIDEO_TYPE_H264) {
            /* h264  .ts */
            rc = ngx_rtmp_mpegts_write_file(file, ngx_rtmp_mpegts_h264_header,
                                        sizeof(ngx_rtmp_mpegts_h264_header));
        } else if (file->video_type == TS_VIDEO_TYPE_H265){
            /* h265  .ts*/
            rc = ngx_rtmp_mpegts_write_file(file, ngx_rtmp_mpegts_hevc_header,
